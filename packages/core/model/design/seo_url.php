...
class SeoUrl extends \Model {
	...
	public function deleteSeoUrlByQuery($query) {
		$this->db->query("DELETE FROM `" . DB_PREFIX . "seo_url` WHERE query LIKE '" . $this->db->escape(html_entity_decode((string)$query, ENT_QUOTES, 'UTF-8')) . "'");
	}
	...
	public function getSeoUrlsByQuery($query, $filter = array()) {
	...
	public function getSeoUrlsByKeyword($keyword, $filter = array()) {
