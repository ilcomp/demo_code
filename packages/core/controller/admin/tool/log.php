...
class Log extends \Controller {
	...
	public function index() {		
		...
		$data['download'] = $this->url->link('tool/log/download');
		$data['clear'] = $this->url->link('tool/log/clear');

		$data['log'] = '';

		$file = DIR_LOGS . $this->config->get('system_error_filename');
		...
		$data['content'] = $this->load->view('tool/log', $data);
	...
	public function download() {
		...
		if (file_exists($file) && filesize($file) > 0) {
			...
			$this->response->redirect($this->url->link('tool/log'));
		}
	}
