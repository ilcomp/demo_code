...
class ControllerToolUpgrade extends Controller {
    public function index() {
		...
		$results = $this->model_core_extension->getExtensionInstalls(0, 1000);
		...
		$data['backup'] = $this->url->link('tool/backup');
    	$data['opencart_account'] = 'https://www.opencart.com/index.php?route=account/account';

		$data['content'] = $this->load->view('tool/upgrade', $data);
	...
	public function modified() {
		...
		if (!$json) {
			...
			$json['next'] = str_replace('&amp;', '&', $this->url->link('tool/upgrade/unzip'));
		}
