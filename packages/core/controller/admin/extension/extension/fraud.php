...
class Fraud extends \Controller {
	...
	public function install() {
		...
		if ($this->validate()) {
			$this->model_core_extension->install('fraud', $this->request->get['extension']);

			$this->load->model('core/user_group');

			$this->model_core_user_group->addPermission($this->user->getGroupId(), 'access', 'extension/fraud/' . $this->request->get['extension']);
			$this->model_core_user_group->addPermission($this->user->getGroupId(), 'modify', 'extension/fraud/' . $this->request->get['extension']);
	...
	public function uninstall() {
		...
		if ($this->validate()) {
			$this->model_core_extension->uninstall('fraud', $this->request->get['extension']);
	...
	protected function getList() {
		...
		foreach ($extensions as $key => $value) {
			if (!is_file(DIR_CONTROLLER . 'extension/fraud/' . $value . '.php') && !is_file(DIR_CONTROLLER . 'fraud/' . $value . '.php')) {
				$this->model_core_extension->uninstall('fraud', $value);
		...
		$files = glob(DIR_CONTROLLER . 'extension/fraud/*.php');
