...
class Google extends \Controller {
	...
	public function index() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			$this->model_core_setting->editSetting('analytics_google', $this->request->post, $this->request->get['store_id']);
		...
		$data['actions']['cancel'] = $this->url->link('marketplace/extension', 'user_token=' . $this->session->data['user_token'] . '&type=analytics');
		...
		if (isset($this->request->post['analytics_google_code'])) {
			...
			$data['analytics_google_code'] = $this->model_core_setting->getSettingValue('analytics_google_code', $this->request->get['store_id']);
		}

		if (isset($this->request->post['analytics_google_status'])) {
			$data['analytics_google_status'] = $this->request->post['analytics_google_status'];
		} else {
			$data['analytics_google_status'] = $this->model_core_setting->getSettingValue('analytics_google_status', $this->request->get['store_id']);
		}

		$data['content'] = $this->load->view('extension/analytics/google', $data);
