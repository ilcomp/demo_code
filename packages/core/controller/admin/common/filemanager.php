...
class FileManager extends \Controller {
	...
	public function view() {
		$data = $this->content('common/filemanager/view');
	...
	private function content($output = 'common/filemanager') {
		$this->load->language('common/filemanager');

		if (!isset($this->request->get['directory']) && isset($this->session->data['filemaneger_directory']) && is_dir(DIR_IMAGE . html_entity_decode($this->session->data['filemaneger_directory'], ENT_QUOTES, 'UTF-8') . '/')) {
			$this->request->get['directory'] = $this->session->data['filemaneger_directory'];
		} elseif (isset($this->request->get['directory'])) {
			$this->session->data['filemaneger_directory'] = $this->request->get['directory'];
		} else {
			unset($this->session->data['filemaneger_directory']);
		}

		// Make sure we have the correct directory
		if (isset($this->request->get['directory']) && $this->request->get['directory']) {
			$directory = DIR_IMAGE . html_entity_decode($this->request->get['directory'], ENT_QUOTES, 'UTF-8') . '/';
		} else {
			$directory = DIR_IMAGE;
		}
		...
		if ($directories) {
			...
			foreach ($images as $image) {
				if (substr(str_replace('\\', '/', realpath($image)), 0, utf8_strlen(DIR_IMAGE)) == DIR_IMAGE) {
					$name = basename($image);
					...
					$data['directories'][] = array(
						...
						'type' => 'directory',
						'href' => $this->url->link($output, 'directory=' . urlencode(utf8_substr($image, utf8_strlen(DIR_IMAGE))) . $url)
					);
				}
			}
		}
		...
		if (!preg_match('/\.(jpg|jpeg|png|gif|svg|JPG|JPEG|PNG|GIF|SVG)$/', $filter_name)) {
			if (substr($filter_name, -1) != '.') {
				$filter_name = $filter_name . '*.';
			}

			$filter_name = $filter_name . '{jpg,jpeg,png,gif,svg,JPG,JPEG,PNG,GIF,SVG}';
		}
		...
		if ($files) {
			...
			foreach ($images as $image) {
				if (substr(str_replace('\\', '/', realpath($image)), 0, utf8_strlen(DIR_IMAGE)) == DIR_IMAGE) {
					...
					$data['files'][] = array(
						'thumb' => $this->model_tool_image->resize(utf8_substr($image, utf8_strlen(DIR_IMAGE)), $thumb_width, $thumb_height),
						'name' => $name,
						'size' => $size ? $size[0] . 'x' . $size[1] : '',
						'path' => utf8_substr($image, utf8_strlen(DIR_IMAGE)),
						'href' => $this->model_tool_image->link(utf8_substr($image, utf8_strlen(DIR_IMAGE)))
					);
				}
			}
		}
		...
		$data['file_upload'] = str_replace('&amp;', '&',$this->url->link('common/filemanager/upload'));
		$data['folder'] = str_replace('&amp;', '&',$this->url->link('common/filemanager/folder'));
		$data['file_delete'] = str_replace('&amp;', '&',$this->url->link('common/filemanager/delete'));
	...
	public function get_link() {
		if ($this->request->get['image']) {
			...
			$image = $this->model_tool_image->link($this->request->get['image']);
		} else {
			$image = '';
		}
