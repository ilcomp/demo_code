...
class Marketplace extends \Controller {
	public function index() {
		...
		$data['content'] = $this->load->view('marketplace/marketplace_list', $data);
	...
	public function info() {
		...
		$data['actions']['cancel'] = $this->url->link('marketplace/marketplace', 'user_token=' . $this->session->data['user_token'] . $url);
			...
			if ($response_info['downloads']) {
				foreach ($response_info['downloads'] as $result) {
					$extension_install_info = $this->model_core_extension->getExtensionInstallByExtensionDownloadId($result['extension_download_id']);
			...
			$data['content'] = $this->load->view('marketplace/marketplace', $data);
		...
		if (!$json) {
			...
			if (isset($response_info['download'])) {
				if (substr($response_info['filename'], -10) == '.ocmod.zip') {
					...
					$json['extension_install_id'] = $this->model_core_extension->addExtensionInstall($response_info['extension'], $extension_id, $extension_download_id);
