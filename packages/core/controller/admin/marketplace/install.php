...
class Install extends \Controller {
				...
				if (!$json) {
					...
					foreach ($files as $file) {
						...
						if (is_dir($file) && !is_dir($path)) {
							if (mkdir($path, 0777)) {
								$this->model_core_extension->addExtensionPath($extension_install_id, $destination);
							}
						}
				...
				if ($xml) {
					try {
						...
						if ($code) {
							...
							$modification_info = $this->model_core_modification->getModificationByCode($code);
	
							if ($modification_info) {
								$this->model_core_modification->deleteModification($modification_info['modification_id']);
							}
						} else {
						...
						if (!$json) {
							...
							$this->model_core_modification->addModification($modification_data);
						}
	...
	public function uninstall() {
		...
		if (!$json) {
			...
			$results = $this->model_core_extension->getExtensionPathsByExtensionInstallId($extension_install_id);
			...
			foreach ($results as $result) {
				...
				$this->model_core_extension->deleteExtensionPath($result['extension_path_id']);
			}

			// Remove the install
			$this->model_core_extension->deleteExtensionInstall($extension_install_id);
			
			// Remove any xml modifications
			$this->load->model('core/modification');

			$this->model_core_modification->deleteModificationsByExtensionInstallId($extension_install_id);
