...
class Translation extends \Controller {
	...
	protected function getList() {
		...
		$data['actions']['add'] = $this->url->link('design/translation/add', 'user_token=' . $this->session->data['user_token'] . $url);
		$data['actions']['delete'] = $this->url->link('design/translation/delete', 'user_token=' . $this->session->data['user_token'] . $url);
		...
		$data['pagination'] = $this->load->controller('block/pagination', array(
			...
			'page'  => $page,
			'limit' => $this->config->get('config_limit'),
			'url'   => $this->url->link('design/translation', 'user_token=' . $this->session->data['user_token'] . '&page={page}')
		));
		...
		$data['content'] = $this->load->view('design/translation_list', $data);
	...
	protected function getForm() {
		...
		$data['actions']['cancel'] = $this->url->link('design/translation', 'user_token=' . $this->session->data['user_token'] . $url);
		...
		$data['stores'] = $this->model_core_store->getStores();
		...
		$data['content'] = $this->load->view('design/translation_form', $data);
	...
	public function path() {
		...
		if (!empty($language_info)) {
			$path = glob(DIR_APP . 'language/*/' . $language_info['code'] . '/*');
			...
			while (count($path) != 0) {
				...
				foreach ((array)glob($next) as $file) {
					...
					if (substr($file, -4) == '.php') {
						$json[] = substr(substr($file, strlen(DIR_APP . 'language/')), 0, -4);
					}
				}
			}
		}
	...
	public function translation() {
		...
		$directory = DIR_APP . 'language/';

		if ($language_info && is_file($directory . $route . '.php') && substr(str_replace('\\', '/', realpath($directory . $route . '.php')), 0, strlen($directory)) == str_replace('\\', '/', $directory)) {
			$_ = array();

			include($directory . $route . '.php');
