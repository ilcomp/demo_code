...
$registry->set('permission', $permission);

// Permission Register
if ($config->has('action_permission')) {
	foreach ($config->get('action_permission') as $key => $permissions) {
		foreach ($permissions as $permission) {
			$permission->add($key, $permission);
		}
	}
}
...
$response->addHeader($config->get('response_content_type'));
$response->setCompression($config->get('response_compression'));
$registry->set('response', $response);

// Database
if ($config->get('db_autostart') && defined('DB_DRIVER')) {
...
// Language Autoload
if (defined('DIR_LANGUAGE') && $config->has('language_autoload')) {
