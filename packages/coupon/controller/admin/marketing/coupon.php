<?php
namespace Controller\Marketing;
...
class Coupon extends \Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$this->response->redirect($this->url->link('marketing/coupon', $url));
		}
	...
	protected function getList() {
		...
		$data['actions']['add'] = $this->url->link('marketing/coupon/add', $url);
		$data['actions']['delete'] = $this->url->link('marketing/coupon/delete', $url);
		...
		foreach ($results as $result) {
			$data['coupons'][] = array(
				...
				'edit'       => $this->url->link('marketing/coupon/edit', 'coupon_id=' . $result['coupon_id'] . $url)
			);
		}
		...
		$data['sort_name'] = $this->url->link('marketing/coupon', '&sort=name' . $url);
		$data['sort_code'] = $this->url->link('marketing/coupon', 'sort=code' . $url);
		$data['sort_discount'] = $this->url->link('marketing/coupon', 'sort=discount' . $url);
		$data['sort_date_start'] = $this->url->link('marketing/coupon', 'sort=date_start' . $url);
		$data['sort_date_end'] = $this->url->link('marketing/coupon', 'sort=date_end' . $url);
		$data['sort_status'] = $this->url->link('marketing/coupon', 'sort=status' . $url);
		...
		$data['pagination'] = $this->load->controller('block/pagination', array(
			...
			'page'  => $page,
			'limit' => $this->config->get('config_limit'),
			'url'   => $this->url->link('marketing/coupon', $url)
		));
		...
		$data['content'] = $this->load->view('marketing/coupon_list', $data);
	...
	protected function getForm() {
		...
		if (!isset($this->request->get['coupon_id'])) {
			$data['action'] = $this->url->link('marketing/coupon/add', $url);
		} else {
			$data['action'] = $this->url->link('marketing/coupon/edit', 'coupon_id=' . $this->request->get['coupon_id'] . $url);
		}

		$data['actions']['save'] = true;
		$data['actions']['cancel'] = $this->url->link('marketing/coupon', $url);
		...
		if (isset($this->request->post['uses_account'])) {
			$data['uses_account'] = $this->request->post['uses_account'];
		} elseif (!empty($coupon_info)) {
			$data['uses_account'] = $coupon_info['uses_account'];
		} else {
			$data['uses_account'] = 1;
		}
		...
		$data['content'] = $this->load->view('marketing/coupon_form', $data);
	...
	public function history() {
		...
		$data['pagination'] = $this->load->controller('block/pagination', array(
			...
			'page'  => $page,
			'limit' => $this->config->get('config_limit'),
			'url'   => $this->url->link('marketing/coupon/history', $url)
		));
