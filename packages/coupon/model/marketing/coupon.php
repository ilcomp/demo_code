...
class Coupon extends \Model {
	...
	public function getCouponHistories($coupon_id, $start = 0, $limit = 10) {
		...
		$query = $this->db->query("SELECT ch.order_id, c.name AS account, ch.amount, ch.date_added FROM " . DB_PREFIX . "coupon_history ch LEFT JOIN " . DB_PREFIX . "account c ON (ch.account_id = c.account_id) WHERE ch.coupon_id = '" . (int)$coupon_id . "' ORDER BY ch.date_added ASC LIMIT " . (int)$start . "," . (int)$limit);
