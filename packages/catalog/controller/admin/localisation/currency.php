...
class Currency extends \Controller {
	...
	protected function getList() {
		...
		$data['actions']['add'] = $this->url->link('localisation/currency/add', 'user_token=' . $this->session->data['user_token'] . $url);
		$data['actions']['delete'] = $this->url->link('localisation/currency/delete', 'user_token=' . $this->session->data['user_token'] . $url);
		$data['actions']['refresh'] = $this->url->link('localisation/currency/refresh', 'user_token=' . $this->session->data['user_token'] . $url);
		...
		foreach ($data['currencies'] as &$result) {
			$result['name'] = $result['name'] . (($result['code'] == $this->config->get('catalog_currency')) ? $this->language->get('text_default') : null);
			$result['date_modified'] = date($this->language->get('date_format_short'), strtotime($result['date_modified']));
			$result['edit'] = $this->url->link('localisation/currency/edit', 'user_token=' . $this->session->data['user_token'] . '&currency_id=' . $result['currency_id'] . $url);
		}
		unset($result);
		...
		$data['sort_name'] = $this->url->link('localisation/currency', 'user_token=' . $this->session->data['user_token'] . '&sort=name' . $url);
		...
		$data['content'] = $this->load->view('localisation/currency_list', $data);
	...
	protected function getForm() {
		...
		$data['actions']['cancel'] = $this->url->link('localisation/currency', 'user_token=' . $this->session->data['user_token'] . $url);
		...
		$data['content'] = $this->load->view('localisation/currency_form', $data);
	...
	protected function validateDelete() {
		...
		foreach ($this->request->post['selected'] as $currency_id) {
			...
			if ($currency_info) {
				if ($this->config->get('catalog_currency') == $currency_info['code']) {
					$this->error['warning'] = $this->language->get('error_default');
				}

				$store_total = $this->model_core_store->getTotalStoresByCurrency($currency_info['code']);
