	...
	public function index() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validate()) {
			if (!isset($this->request->get['module_id'])) {
				$this->model_core_module->addModule('info_article', $this->request->post);
			} else {
		...
		if (!isset($this->request->get['module_id'])) {
			$data['breadcrumbs'][] = array(
				...
				'href' => $this->url->link('extension/module/info_article')
			);
		} else {
			$data['breadcrumbs'][] = array(
				'text' => $this->language->get('heading_title'),
				'href' => $this->url->link('extension/module/info_article', 'module_id=' . $this->request->get['module_id'])
			);
		}

		if (!isset($this->request->get['module_id'])) {
			$data['action'] = $this->url->link('extension/module/info_article');
		} else {
			$data['action'] = $this->url->link('extension/module/info_article', 'module_id=' . $this->request->get['module_id']);
		}
		...
		$data['content'] = $this->load->view('extension/module/info_article', $data);

		$this->response->setOutput($this->load->controller('common/template', $data));
	}

	protected function validate(){
		if (!$this->user->hasPermission('modify', 'extension/module/info_article')) {
