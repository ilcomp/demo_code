...
class Pagination extends \Controller {
	public function index($setting) {
		...
		if ($num_pages > 1) {
			return $this->load->view('block/pagination', $data);
		} else {
			return '';
		}
	}

	public function result($setting) {
		if (isset($setting['total'])) {
			$total = (int)$setting['total'];
		} else {
			$total = 0;
		...
		return sprintf($this->language->get('text_pagination'), $total ? ($page - 1) * $limit + 1 : 0, ($page * $limit - $total > 0) ? $total : ($page * $limit), $total, ceil($total / $limit));
	}
}
