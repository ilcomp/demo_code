...
class AccountGroup extends \Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			$this->response->redirect($this->url->link('account/account_group', 'user_token=' . $this->session->data['user_token'] . $url));
		}
	...
	protected function getList() {
		...
		$data['breadcrumbs'][] = array(
			...
			'href' => $this->url->link('account/account_group', 'user_token=' . $this->session->data['user_token'] . $url)
		);

		$data['actions']['add'] = $this->url->link('account/account_group/add', 'user_token=' . $this->session->data['user_token'] . $url);
		$data['actions']['delete'] = $this->url->link('account/account_group/delete', 'user_token=' . $this->session->data['user_token'] . $url);
		...
		$data['account_groups'] = $this->model_account_account_group->getAccountGroups($filter_data);

		foreach ($data['account_groups'] as &$result) {
			$result['name'] = $result['name'] . (($result['account_group_id'] == $this->config->get('account_group_id')) ? $this->language->get('text_default') : null);
			$result['edit'] = $this->url->link('account/account_group/edit', 'user_token=' . $this->session->data['user_token'] . '&account_group_id=' . $result['account_group_id'] . $url);
		}
		unset($result);
		...
		$data['sort_name'] = $this->url->link('account/account_group', 'user_token=' . $this->session->data['user_token'] . '&sort=cgd.name' . $url);
		$data['sort_sort_order'] = $this->url->link('account/account_group', 'user_token=' . $this->session->data['user_token'] . '&sort=cg.sort_order' . $url);
		...
		$data['pagination'] = $this->load->controller('block/pagination', array(
			'total' => $account_group_total,
			'page'  => $page,
			'limit' => $this->config->get('config_limit'),
			'url'   => $this->url->link('account/account_group', $url)
		));
		...
		$data['content'] = $this->load->view('account/account_group_list', $data);
	...
	protected function getForm() {
		...
		if (!isset($this->request->get['account_group_id'])) {
			$data['action'] = $this->url->link('account/account_group/add', 'user_token=' . $this->session->data['user_token'] . $url);
		} else {
			$data['action'] = $this->url->link('account/account_group/edit', 'user_token=' . $this->session->data['user_token'] . '&account_group_id=' . $this->request->get['account_group_id'] . $url);
		}

		$data['actions']['save'] = true;
		$data['actions']['cancel'] = $this->url->link('account/account_group', 'user_token=' . $this->session->data['user_token'] . $url);
		...
		$data['content'] = $this->load->view('account/account_group_form', $data);
	...
	protected function validateDelete() {
		...
		foreach ($this->request->post['selected'] as $account_group_id) {
			if ($this->config->get('account_group_id') == $account_group_id) {
				$this->error['warning'] = $this->language->get('error_default');
			}

			$store_total = $this->model_core_store->getTotalStoresByAccountGroupId($account_group_id);
