...
class User extends \Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			$this->model_core_user->addUser($this->request->post);
	...
	public function edit() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			$this->model_core_user->editUser($this->request->get['user_id'], $this->request->post);
	...
	public function delete() {
		...
		if (isset($this->request->post['selected']) && $this->validateDelete()) {
			foreach ($this->request->post['selected'] as $user_id) {
				$this->model_core_user->deleteUser($user_id);
			}
	...
	protected function getList() {
		...
		$data['actions']['add'] = $this->url->link('user/user/add', 'user_token=' . $this->session->data['user_token'] . $url);
		$data['actions']['delete'] = $this->url->link('user/user/delete', 'user_token=' . $this->session->data['user_token'] . $url);
		...
		$user_total = $this->model_core_user->getTotalUsers();

		$results = $this->model_core_user->getUsers($filter_data);
		...
		$data['content'] = $this->load->view('user/user_list', $data);
	...
	protected function getForm() {
		...
		$data['actions']['cancel'] = $this->url->link('user/user', 'user_token=' . $this->session->data['user_token'] . $url);

		if (isset($this->request->get['user_id']) && ($this->request->server['REQUEST_METHOD'] != 'POST')) {
			$user_info = $this->model_core_user->getUser($this->request->get['user_id']);
		}
		...
		$data['user_groups'] = $this->model_core_user_group->getUserGroups();
		...
		if (isset($this->request->post['image']) && is_file(DIR_IMAGE . $this->request->post['image'])) {
			...
			$data['thumb'] = $this->model_tool_image->resize($user_info['image'], 200, 200);
		} else {
		...
		$data['content'] = $this->load->view('user/user_form', $data);
	...
	protected function validateForm() {
		...
		if ((utf8_strlen($this->request->post['username']) < 3) || (utf8_strlen($this->request->post['username']) > 255)) {
			$this->error['username'] = $this->language->get('error_username');
		}

		$user_info = $this->model_core_user->getUserByUsername($this->request->post['username']);
		...
		$user_info = $this->model_core_user->getUserByEmail($this->request->post['email']);
