...
class Event extends \Model {
	...
	public function getEvents($data = array()) {
		$sql = "SELECT * FROM `" . DB_PREFIX . "event` WHERE 1";

		if (isset($data['filter_code'])) {
			$sql .= " AND `code` LIKE '" . $this->db->escape($data['filter_code']) . "%'";
		}

		if (isset($data['filter_trigger'])) {
			$sql .= " AND `trigger` LIKE '" . $this->db->escape($data['filter_trigger']) . "%'";
		}
	...
	public function getTotalEvents($data = array()) {
		$sql = "SELECT COUNT(*) AS total FROM `" . DB_PREFIX . "event` WHERE 1";
