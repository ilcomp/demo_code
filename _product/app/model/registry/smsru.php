...
class SmsRu {
    ...
    private $sign = ''; //Подпись по умолчанию
    private $status = false; //Статус
    ...
    private function curl_post($url, array $post = NULL, array $options = array()){
        ...
        $this->status = isset($return['status_code']) && $return['status_code'] == 100;
    ...
    public function send($number, $text, $channel = '', $dateSend = null, $callbackUrl = null){
        $return = $this->curl_post($this->url . '/sms/send', [
            'to' => is_array($number) ? implode(',', $number) : $number,
            'from' => $this->sign,
            'msg' => $text,
            'time' => $dateSend
        ]);
        ...
        if (isset($return['sms']))
            foreach ($return['sms'] as $key => $value) {
                if (isset($value['sms_id'])) {
                    $data[] = array(
                        "id" => (int)preg_replace('/\D/', '', $value['sms_id']),
                        "number" => $key,
                        "status" => $return['status_code'] == 100
                    );
                } else {
                    $this->log_sms->write(json_encode($value));
                }
            }
    ...
    public function check_send($id){
        return $this->curl_post($this->url . '/sms/status/', [
            'sms_id' => $id
        ]);
    }
    ...
    public function balance(){
        $return = $this->curl_post($this->url . '/my/balance', []);
